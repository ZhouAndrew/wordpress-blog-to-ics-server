...
def publish_post(config: AppConfig, post_id: int, verbose: bool = False) -> dict[str, Any] | None:
    """Publish one WordPress post through the canonical source → parser → exporter pipeline.

    Returns a publish-index-compatible item dictionary, or ``None`` when the
    post has no exportable timed entries after review filtering.
    """
    _phase(verbose, "fetch", f"fetching WordPress post {post_id}")
    post = fetch_post(config, post_id)
...
def publish_once(config: AppConfig, days: int, verbose: bool = False) -> dict[str, Any]:
    ...
    for post_id in post_ids:
        item = publish_post(config, post_id, verbose=verbose)
        if item is not None:
            items.append(item)
